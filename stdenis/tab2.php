...
//pending tickets
					...
					$con = @mysqli_connect($host,$user,$password,$database) or die('Connect Error: ' . mysqli_connect_errno());;
					    ...
					    if(isset($_POST['page']))
						{
						    ...
						    $query = "SELECT * FROM tickets WHERE date = '".date('Y-m-d')."' AND complete = 0 ORDER BY timeIn LIMIT ".$page.",".$itemPerPage;
						     
						}
						else
						{
						    //get from the beginning $itemPerPage elements
					       $query = "SELECT * FROM tickets WHERE date = '".date('Y-m-d')."' AND complete = 0 ORDER BY timeIn LIMIT 0,".$itemPerPage;
						...
						$query2 = "SELECT * FROM tickets WHERE date = '".date('Y-m-d')."' AND complete = 0 ORDER BY timeIn";
				...
				?>
					   ...
					   ?>
							...
							<tfoot>
								<tr>
									<td colspan="7">
										...
										echo "<div class='pagination' id='pagination2'>
						...
						else
						{
								...
								?>
								<script> 
							    document.getElementById("messageDatabasePending").innerHTML = "The Database is empty at this point.";
