...
//all tickets
					...
					$con = @mysqli_connect($host,$user,$password,$database) or die('Connect Error: ' . mysqli_connect_errno());;
					    ...
					    if(isset($_POST['page']))
						{
						    ...
						    $query = "SELECT * FROM tickets WHERE date = '".date('Y-m-d')."' ORDER BY timeIn LIMIT ".$page.",5";
						     
						}
						else
						{
					       $query = "SELECT * FROM tickets WHERE date = '".date('Y-m-d')."' ORDER BY timeIn LIMIT 0,5";
						...
						$query2 = "SELECT * FROM tickets WHERE date = '".date('Y-m-d')."' ORDER BY timeIn";
				...
				?>
					   ...
					   ?>
							...
							<tfoot>
								<tr>
									<td colspan="7">
										...
										echo "<div class='pagination' id='pagination1'>
						...
						else
						{
								...
								?>
								<script> 
							    document.getElementById("messageDatabaseAll").innerHTML = "The Database is empty at this point.";
